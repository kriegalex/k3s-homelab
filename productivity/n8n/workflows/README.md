...
| `appt-21-enrich.json` | For `email-*` listings: fetch the detail page via `tls-fetch` (curl_cffi), backfill description/amenities/floor/GPS/images/agency, join `agency_reputation` | called by APPT 20 |
| `appt-50-daily-digest.json` | 08:00 Telegram digest of last-24 h listings | cron `0 8 * * *` |
